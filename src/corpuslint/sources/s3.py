...
def _import_boto3():
    ...
    never requires the optional extra). Returns ``(boto3, retryable_errors,
    NoCredentialsError)`` where ``retryable_errors`` is the tuple of boto/botocore
    exception classes to translate into a clean :class:`S3Error`."""
    try:
        import boto3
...
def _documents_from_client(client, bucket: str, prefix: str, config: Config) -> list[Document]:
...
def load_s3_documents(
    ...
    """
    boto3, retryable_errors, no_credentials_error = _import_boto3()
    client_kwargs: dict = {}
    if endpoint_url:
    ...
    try:
        docs = _documents_from_client(client, bucket, prefix, config)
        ...
        # Report the failure class only — boto/botocore messages can echo the
        # bucket, region or request detail, so keep them out of user-facing text.
        raise S3Error(
